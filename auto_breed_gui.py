"""
PC Fish 智能繁殖與合成管理終端 v1.1.4 STABLE
...
- 🌟 純記憶體原生直發合成/融合訊號 (三重旁路補丁 + 官方實例填入 + 觸控阻擋自動解除，徹底杜絕畫面定格卡死)
...
class AutoBreedApp:
    def __init__(self, root):
        self.root = root
        self.root.title("PC Fish 智能繁殖與合成管理終端 v1.1.4 STABLE (純記憶體直發)")
        ...
        while self.is_monitoring:
            try:
                ...
                class_res = self.mem.get_season_and_general_classification(
                    ...
                    max_merge_star=self.opt_max_merge_star.get()
                )
