...
6. **🌟 官方賽季魚 1~5 星合成雷達 (Season Crafting Radar)**
   - 嚴格逆向對齊遊戲官方 CSV `fish_season_recipe_v1.0` 配方：
     - **1 星**：3 種同階素材各 3 隻（共 9 隻）+ 1 隻高一階素材（共 1 隻）= 10 隻。
     - **2~5 星**：前一星級賽季魚（1 隻）+ 原 3 種同階素材各 3 隻（隨星數成長同星級）= 10 隻。
   - 支援「霜藍翻車魚 (FS00033)」、「萊姆背海龜 (FS00034)」與「祭典章魚 (FS00035)」全星級進度盤點與缺口統計。
   - 自動鎖定賽季合成所需魚隻並加入保護名單（`Reserved Fish`），嚴格防範被挪做他用。

7. **⚡ 純記憶體一般魚融合分流與 0 次廢魚優先耗損 (Pure Memory Fish Merge)**
   - 智慧過濾將「非賽季目標魚種」（如 FS00001~FS00006）與「賽季溢出多餘材料」分流至一般魚融合池。
   - **0 次廢魚優先**：嚴格依據繁殖次數遞增排序（`breed == 0` 優先），將廢魚優先轉化為高階魚隻。
   - **安全防禦機制**：嚴格限制素材稀有度上限（預設僅普通與高級，最高支援稀有），杜絕傳說與神話魚被誤融。
   - **純記憶體發送**：直接定位 `UIMerge` 物件並以 IL2CPP 線程原生調用 `UIMerge.Merge`，零滑鼠移動、背景靜默發送。
...
### `v1.1.5 STABLE`
* **【賽季合成修復】** 深度反彙編排查 `UIMerge.Merge` (`0x5BD580`) 與 `NetworkManager.FishSeasonCraft` (`0x5F10C0`)，修正託管 `FishTypeModel` 物件構建，徹底修復「播放音效但伺服端未扣除材料」的根本問題。
* **【材料星級校正】** 修正材料分類時誤將稀有度代碼當作星級的錯誤，精準匹配 1~5 星賽季材料需求。
* **【伺服端握手驗證】** 引入 3.5 秒動態輪詢伺服端材料扣除握手確認，即時監控背包消耗，徹底杜絕假成功。
* **【全域觸控安全鎖】** 自動在合成與特效結算後清除 `UIManager.isTouchBlock`，防止介面卡死無響應。
* **【自主保護配置】** 支援一般融合自主設定星級上限（預設 <= 3 星）與稀有度上限（預設 <= 高級），防護高階魚種。
